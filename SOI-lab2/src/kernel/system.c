...
FORWARD _PROTOTYPE( int do_getprocpri, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_setprocpri, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_SETGROUP:  r = do_setprocpri(&m);	break;
	    case SYS_GETGROUP:  r = do_getprocpri(&m);  break;
	    default:		r = E_BAD_FCN;
	}
...
PRIVATE int do_getprocpri(m_ptr)
message *m_ptr;
{
  struct proc *p;

  for(p = BEG_PROC_ADDR; p < END_PROC_ADDR; p++){
    if(istaskp(p) || isservp(p))
      continue;
...
PRIVATE int do_setprocpri(m_ptr)
message *m_ptr;
{
  struct proc *p;
