...
FORWARD _PROTOTYPE( void changeProc, (struct proc *p) );
 ...
 *				sched					     * 
 *===========================================================================*/
PRIVATE void changeProc(p)
register struct proc *p;
{
	register struct proc *curr, *next;
	curr = rdy_head[USER_Q];
	while(curr->p_nextready != p){
		curr = curr->p_nextready;
	}
	next = curr->p_nextready->p_nextready;
	curr->p_nextready->p_nextready = rdy_head[USER_Q];
	rdy_head[USER_Q] = curr->p_nextready;
	curr->p_nextready = next;
}
 ...
 */
  register struct proc *pri, *norm, *backg, *curr;
  int prev_group, count = 0;
  
  if (rdy_head[USER_Q] == NIL_PROC) return;
  
  rdy_head[USER_Q]->counter++;
  if(rdy_head[USER_Q]->counter <= count){
	pick_proc();
	return;
  }
  
  rdy_head[USER_Q]->counter = 0;
  prev_group = rdy_head[USER_Q]->group;
  ...
  curr = rdy_head[USER_Q];
  pri = norm = backg = NIL_PROC;

  while(curr != NIL_PROC){
	if(pri == NIL_PROC && curr->group == PRIORITY){
		pri = curr;
		continue;
	} if(norm == NIL_PROC && curr->group == NORMAL){
		norm = curr;
		continue;
	} if(backg == NIL_PROC && curr->group == BACKG){
		backg = curr;
	}
	curr = curr->p_nextready;
  }
/*przypadki, ze istnieja procesy uprzywilejowane i zwykle oraz procesy tla istnieja badz nie*/
  if(pri != NIL_PROC && norm != NIL_PROC){
	count = 2;	
	if(prev_group == PRIORITY){
		if(norm == rdy_head[USER_Q]){
			pick_proc();
			return;
		} else{
			changeProc(norm);
...
/*istnieja tylko procesy uprzywilejowane i tla*/
  if(pri != NIL_PROC && backg != NIL_PROC){
	count = 1;	
	if(pri == rdy_head[USER_Q]){
		pick_proc();
		return;
...
/*istnieja tylko procesy normalne i tla*/
  if(norm != NIL_PROC && backg != NIL_PROC){
	count = 1;
	if(norm == rdy_head[USER_Q]){
		pick_proc();
		return;
...
/*zwykle wykonywanie*/
  if( (pri != NIL_PROC && norm == NIL_PROC && backg == NIL_PROC) || 
	(pri == NIL_PROC && norm != NIL_PROC && backg == NIL_PROC) ||
	(pri == NIL_PROC && norm == NIL_PROC && backg != NIL_PROC) ){
	count = 1;	
	pick_proc();
	return;
