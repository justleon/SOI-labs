...
#define FIRST_FIT_MODE 0
#define WORST_FIT_MODE 1
...
PRIVATE int algMode = FIRST_FIT_MODE; /* which mode are we using? */
...
{
	if(mm_in.m1_i1 == FIRST_FIT_MODE || mm_in.m1_i1 == WORST_FIT_MODE)
		algMode = mm_in.m1_i1;
 ...
 */
  ...
  else if(algMode == WORST_FIT_MODE){ /* zaimplementowany algorytm worst fit */
  do {
  	hp = hole_head;
  	max = hole_head;
  	prevmax = NIL_HOLE;
	...
	if(hp == NIL_HOLE && max->h_len >= clicks){
		/* using memory as in first fit algorthm */
		old_base = max->h_base;
		max->h_base += clicks;
		max->h_len -= clicks;
